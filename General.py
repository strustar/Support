...
def Tab(In):
    ...
    word_wrap_style(s1, txt, In.font_h5)    
    txt = '① 시공사에서 제시한 시공조건 및 도면을 근거로 검토하였음. 따라서, :blue[현장 여건이 변경되는 경우에는 반드시 검토자와 협의 후 시공]한다.'
    word_wrap_style(s1, txt, In.font_h5)
    txt = '② 본 공사는 ":blue[5. 적용기준 및 참고문헌]"에 제시된 설계기준 및 시공기준을 따라 시공하여야 하며, 거푸집 및 동바리에 적용되는 각종 안전작업지침 및 설치지침에 따라 시공한다.'
    word_wrap_style(s1, txt, In.font_h5)
    txt = '③ 모든 재료적 성능은 검토서에 표기된 :blue[동등 이상의 제품을 확인]하고 설치한다.'
    word_wrap_style(s1, txt, In.font_h5)
    txt = '④ 경사지에 설치되는 구조용 :blue[동바리는 수직을 유지]하게 설치한다.'
    word_wrap_style(s1, txt, In.font_h5)    
    txt = '⑤ 슬래브에 설치되는 합판은 주변의 벽체 및 기둥 등에 :blue[견고하게 밀착되도록 설치]한다.'
    word_wrap_style(s1, txt, In.font_h5)
    txt = '⑥ 구조용 동바리의 지지부 하부에는 :blue[침하가 발생하지 않도록] 지반을 다지고 버림 콘크리트를 시공하는 등 관련 조치를 하여 침하가 발생되지 않도록 한다. (특히, 지반 하부에 공동, 배수관 등에 대한 확인 및 조치 필요)'
    word_wrap_style(s1, txt, In.font_h5)    
    txt = '⑦ 각각의 가설재(합판, 장선, 멍에, 수직재, 수평재, 가새재 등)는 서로 :blue[견고하게 결속하여 미끄러지거나 변형이 발생되지 않도록] 한다.'
    word_wrap_style(s1, txt, In.font_h5)    
    txt = '⑧ 가설 구조물 양측에 강성이 큰 구조물이 존재할 경우에는 직접 이에 지지하여 수평변위를 최대한 방지하며, 특히 콘크리트 부분 타설 등 상부 편심하중에 의한 횡방향 쏠림 현상이 크게 발생할 우려가 있는 시공 조건일 경우 이를 미연에 방지할 수 있도록 경사 버팀대 등으로 충분히 보강한다.'
    word_wrap_style(s1, txt, In.font_h5)    
    txt = '⑨ 본 검토에 적용된 시스템 동바리의 규격 및 물성치는 :blue[현장에 적용되는 제품과 반드시 일치되는지 확인]을 거쳐야 하며, 현장에서는 반입제품의 재사용 가설 기자재 성능저하 안전율을 확인하여 :blue[안정성이 검증된 제품을 설치]하도록 한다.'
    word_wrap_style(s1, txt, In.font_h5)
    ...
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 규격품이나 성능이 확인된 제품을 제외한 거푸집의 경우는 :blue[공인시험기관의 시험값]을 기준으로 한 허용하중값을 적용한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 거푸집은 예상되는 하중조건에 대하여 모든 부속품이 :blue[허용응력을 초과하지 않아야 하며, 변형기준 이하]가 되도록 설계한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 거푸집은 부과되는 연직하중과 수평하중을 지반 또는 영구 구조체에 :blue[안전하게 전달]할 수 있도록 설계한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 목재 거푸집, 장선 및 멍에는 등분포 하중이 작용하는 :blue[단순보로 검토]한다. 다만, 강재나 알루미늄 등과 같은 재료가 사용되는 경우 지점조건에 맞게 설계한다.'
    word_wrap_style(s2, txt, In.font_h5)
    ...
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 동바리 기초는 상부하중에 대한 지반의 :blue[허용지지력 및 허용침하량을 초과하지 않도록 설계] 하며, 동바리의 모든 부품 및 부속품이 :blue[변형기준과 허용응력을 초과하지 않도록 설계]한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 동바리의 설계는 시공 중과 완성 후의 :blue[전체 연직방향 변위량에 충분한 안전성을 확보] 하며, 이때 전체 연직방향 변위량은 기초 침하량과 동바리 자체 변형량을 포함한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 양중이 필요한 동바리는 :blue[양중에 의한 영향을 고려]한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 동바리에 설치되는 수평재 및 가새재는 예상되는 :blue[모든 수평하중을 안전하게 지지]할 수 있도록 설치한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 동바리 시공 중 태풍 등과 같은 강풍이 작용하여 동바리가 붕괴될 우려가 있는 경우에는 수평방향 풍하중에 저항할 수 있도록 설계하며, 특히 콘크리트 부분 타설 등 상부 편심하중에 의해 횡방향 쏠림현상(sidesway)이 크게 발생할 우려가 있는 시공조건일 경우 이를 미연에 방지할 수 있는 :blue[경사버팀대 등으로 견고하게 보강]한다.'
    word_wrap_style(s2, txt, In.font_h5)
    txt = '￭ 건물의 층고 및 부재의 높이가 높아 단품지지 동바리를 사용할 수 없는 경우에는 :blue[현장 여건에 적합한 동바리로 설계]한다.'
    word_wrap_style(s2, txt, In.font_h5)
