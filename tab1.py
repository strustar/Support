...
def Tab(In, color, fn, Wood, Joist, Yoke):
    ...
    In.design_load = table.Load(fn, In.thick_height, In.concrete_weight, In.wood_weight)/1e3    
...
def Check(In, opt, fn, Wood, Joist, Yoke):
    ...
    if '합판' in opt[0]:
        ...
        w_str = 'ω_w';  img = 'wood.png';  AIb_Q = Ib_Q;  L = [1, In.Lj]
        
    if '장선' in opt[0]:
        [A, I, S, E, fba, fsa, section] = [Joist.A, Joist.I, Joist.S, Joist.E, Joist.fba, Joist.fsa, In.joist]
        color = 'green';  L_jyv = rf'\small{{\textcolor{{{color}}}{{L_y}}}}'
        w_str = 'ω_j';  img = 'joist.png';  AIb_Q = A;  L = [In.Lj, In.Ly]

    if '멍에' in opt[0]:
        [A, I, S, E, fba, fsa, section] = [Yoke.A, Yoke.I, Yoke.S, Yoke.E, Yoke.fba, Yoke.fsa, In.yoke]
        color = 'blue';  L_jyv = rf'\textcolor{{{color}}}{{L_v}}'    
        w_str = 'ω_y';  img = 'yoke.png';  AIb_Q = A;  L = [In.Ly, In.Ly]

    w = In.design_load*L[0]
    L_jyv1 = opt[1] + rf'($\bm{{{L_jyv}}}$) 검토'
    L_jyv2 = rf'$\bm{{{{{L_jyv}}} }} \bm{{\small{{\textcolor{{{color}}}{{\: = {L[1]:0.1f} \pmb{{\rm{{\: mm}}}} }}}} }}$'
    st.write(h4, opt[2] + opt[0] + ' 및 ' + L_jyv1)
    table.Info(fn, opt[0], section, AIb_Q, I, S, E, fba, fsa, 20)
    ...
    st.write(s3, rf'$\bm{{\small{{\quad M_{{max}}}} = \large{{\frac{{{w_str} {L_jyv}^2}}{{8}}}} \small \leq \: f_{{ba}}\,S}}$')
    num_str = rf"$\bm{{\large{{\sqrt{{\frac{{8 \times {fba:.1f} \times {S:,.1f}}}{{{w:.4f}}} }} }} \normalsize = \:\:}}$";  Lm = (8*fba*S/w)**(1/2)
    st.write(s3, rf'$\bm{{\quad {L_jyv} \leq \large\sqrt{{\frac{{8\,f_{{ba}}\,S}}{{{w_str}}}}} \small = \:}} $', num_str + f'{Lm:,.1f} mm')
    ...
    st.write(s3, rf'$\bm{{\quad \delta_{{max}} = \large{{\frac{{5\,{{{w_str}}} {L_jyv}^4}}{{384\,E\,I}}}} \small{{\leq }}}} $', In.d1_str)
