...
def Check(In, opt, fn, s1, s2, s3, h4, h5, Wood, Joist, Yoke, t_load, level, d1, d2, Ln, L):
    ...
    load_str = opt[0] + f'에 재하되는 하중 ($\pmb{{{w_str}}}$) [폭 : {width_str}]'  #이, 가 로 안됨 ㅠㅠㅠ    
    L_str = rf'\textcolor{{{color}}}' + txt
    st.write(h4, title + f' ($\small\pmb{{{L_str}}}$) 결정')
    ...
    with col1:
        ...
        st.write(s2, f'➣ $\pmb{{{w_str}}}$ = 설계 하중 x ' + width_str)
        precision = 4 if '합판' in opt[0] else 2
        st.write(s2, f'➣ $\pmb{{{w_str}}} = {t_load:0.4f}$ N/mm² x  {L:0.1f} mm = {w:.{precision}f} N/mm')
    with col2:
    ...
    st.write(s3, rf'$\pmb{{\quad M = \Large{{\frac{{{w_str}\textcolor{{red}}{{{L_str}}}^2}}{{8}}}} \normalsize \leq f_{{ba}}\,S}} $')    
    Lm = (8*fba*S/w)**(1/2)
    num_str = rf"$\pmb{{\large{{\sqrt{{\frac{{8 \times {fba:.1f} \times {S:,.1f}}}{{{w:.4f}}} }} }} \normalsize = \:}}$"
    st.write(s3, rf'$\pmb{{\quad\textcolor{{red}}{{{L_str}}}\leq \Large\sqrt{{\frac{{8\,f_{{ba}}\,S}}{{{w_str}}}}} \normalsize = \:}} $', num_str + f'{Lm:,.1f} mm')
    ...
    st.write(s3, rf'$\pmb{{\quad \delta = \Large{{\frac{{5\,{{{w_str}}} \textcolor{{red}}{{{L_str}}}^4}}{{384\,E\,I}}}} \normalsize \leq}} $', d1)
    num_str = rf"$\pmb{{\large{{\sqrt[4]{{\frac{{384 \times {E:,.1f} \times {I:,.1f} \times {Ln:,.1f}}}{{5 \times {w:,.4f} \times 360 }}}} }} \normalsize = \:}}$"
    st.write(s3, rf'$\pmb{{\quad\textcolor{{red}}{{{L_str}}} \leq \Large\sqrt[4]{{\frac{{384\,E\,I\,L_n}}{{{{5\,{{{w_str}}}}}\,{{{j1:.0f}}} }}}} \normalsize = \:}} $', num_str + f'{Ld1:,.1f} mm')

    st.write(s3, 'b. 절대변형 기준')
    st.write(s3, rf'$\pmb{{\quad \delta = \Large{{\frac{{5\,{{{w_str}}} \textcolor{{red}}{{{L_str}}}^4}}{{384\,E\,I}}}} \normalsize \leq}} $', d2)
    num_str = rf"$\pmb{{\large{{\sqrt[4]{{\frac{{384 \times {E:,.1f} \times {I:,.1f} \times 3}}{{5 \times {w:,.4f} }}}} }} \normalsize = \:}}$"
    st.write(s3, rf'$\pmb{{\quad\textcolor{{red}}{{{L_str}}} \leq \Large\sqrt[4]{{\frac{{384\,E\,I\,{{{j2:.0f}}}}}{{5\,{{{w_str}}} }}}} \normalsize = \:}} $', num_str + f'{Ld2:,.1f} mm')
    ...
    st.write(s3, '➣ '+opt[1]+' 간격을', rf'$\pmb{{\small \: \textcolor{{blue}}{{{L_str}}} = }}$ :orange['+f'{L:,.0f}', 'mm] 간격으로 설치한다. (:orange[최솟값의]', rf'$\pmb{{\small \textcolor{{orange}}{{{margin:.0f}}} }}$'+':orange[%] 정도로 설정)')
    
    st.write(s2, '④ 전단 검토')
    Vmax = w*L/2;  fs = Vmax/AIb_Q
    fs_str = 'Ib/Q' if '합판' in opt[0] else 'A'

    num_str = rf'$\pmb{{\large{{\frac{{ {w:,.4f} \times {L:,.1f}}}{{2}} }} \normalsize = \:}}$'
    st.write(s3, rf'$\pmb{{\quad V_{{max}} = \Large{{\frac{{ {{{w_str}}}\,\textcolor{{red}}{{{L_str}}}}} {{2}}}} \normalsize =}}$', num_str + f'{Vmax:,.2f} N')
    num_str = rf'$\pmb{{\large{{\frac{{ {Vmax:,.1f}}}{{{AIb_Q:,.1f}}} }} \normalsize = \:}}$'
    okng = ':blue[OK]' if fs <= fsa else ':red[NG]'
    st.write(s3, rf'$\pmb{{\quad f_s = \Large{{\frac{{V_{{max}}}}{{{fs_str}}} }} \normalsize =}}$', num_str, f'{fs:.2f} MPa $\pmb\leq$ {fsa:.2f} MPa', r'$\pmb {( = f_{sa})} \qquad$', okng)
