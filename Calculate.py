...
def Info(In, color, Wood, Joist, Yoke, Vertical, Horizontal, Bracing):
    ...
    st.write(s1, '2) 수평하중 (H)' + '$\qquad$ :orange[ <근거 : 1.6.5 수평하중 (KDS 21 50 00 : 2022)>]')    
...
def Check_Support(In, opt, section, Support):
    ...
    st.write(s2, rf'➣ 유효 좌굴길이  $\; : \;$ $\bm{{\small{{{KL_str}}} }} = $', KL_cal, f'{KL:,.1f} mm' + '$\qquad$ :orange[ <근거 : 4.4.2 좌굴길이와 세장비 (KDS 14 30 10 : 2019)>]')
    num_str = rf'$\bm{{\large{{\frac{{{KL:,.1f}}}{{{r:,.1f}}}}} }} \; = \;$';  lamda = KL/r    
    [lgeq, okng] = ['\leq', In.ok] if lamda <= 200 else ['\geq', In.ng]
    st.write(s2, '➣ 세장비 $\; : \;$ ' + rf'$\bm{{\lambda = \large{{\frac{{{KL_str}}}{{r}}}} }}$ = ' + num_str + f'{lamda:,.1f}', rf'$\; {lgeq} \:$ 200 (최대 세장비) $\qquad$' + okng)
    ...
    st.write('###### $\quad \qquad$', '*단품 동바리 안전율 3.0, 조립식 동바리 안전율 2.5 적용')
...
def Check(In, opt, section, WJY):
    ...
    st.write(s2, '✦ ', L_jyv2, rf'$\bm{{\; {lgeq} \;}}$' + f'{Lm:,.1f} mm $\qquad$' + okng)

    st.write(In.space, unsafe_allow_html=True)  ## 빈줄 공간
    st.write(s1, '4) 변형 검토 (표면 등급 : :blue[', In.level, '] )' + '$\qquad$ :orange[ <근거 : 1.9 변형기준 (KDS 21 50 00 : 2022)>]')
    ...
    st.write(s3, '✦ ', L_jyv2, rf'$\bm{{\; {lgeq} \;}}$' + f'{Ld1:,.1f} mm $\qquad$' + okng)
    ...
    st.write(s3, '✦ ', L_jyv2, rf'$\bm{{\; {lgeq} \;}}$' + f'{Ld2:,.1f} mm $\qquad$' + okng)
